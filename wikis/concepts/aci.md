...
[CLI-Anything](../entities/cli-anything.md) 代表了 ACI 设计的一个激进立场："If you can CLI it, you should CLI it"——把命令行 + `--json` 作为 agent 与软件交互的默认形式，并用 7 阶段自动流水线把这种接口套到任意存量软件上。这一路线与 [cli-vs-gui-automation](cli-vs-gui-automation.md) 中讨论的两条哲学路线相呼应，也把 [agent-native-software](agent-native-software.md) 的产品设计思路引入到现有软件的改造阶段。
...
[Structured Outputs](structured-outputs.md) 将 ACI 的思想推进了一步：不只是"接口文档对 LLM 直觉化"，而是通过 [JSON Schema + 约束解码](constrained-decoding.md)实现**机器可验证的接口契约**。工具的参数格式从"建议"变成"硬约束"——模型物理上无法生成不符合 schema 的 function call 参数。这是 ACI 设计从软性规范走向形式化保证的关键一步。
