...
三种退化，三个名字。乍看是三个独立的问题，需要三组独立的解决方案。
...
这三种现象看起来各有各的名字、各有各的表现，但它们有一个共同的底层语言：信息论。
...
这个统一不只是修辞上的类比。它有一个实质性的含义：既然三种现象本质上是信息通道中的噪声问题，那么对抗它们的逻辑框架就是一致的。理解信息如何在 agent 系统中退化，就掌握了分析所有这些退化的统一工具。
